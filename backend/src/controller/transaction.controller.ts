...
const getSpendingOverview  = asyncHandler(async (req: Request & { user?: IUser }, res) => {
  ...
  const monthLabels = monthlyAggregation.map((m) =>
    `${m._id.year}-${m._id.month.toString().padStart(2, "0")}`
  );
  ...
  for (const category of Object.keys(trendMap)) {
    categoryTrends[category] = allLabels.map((label) =>
      trendMap[category][label] || 0
    );
  }
